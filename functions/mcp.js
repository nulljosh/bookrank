...
// in-page tools in webmcp.js, which read and write the signed-in reader's own summaries.
  ...
  switch (method) {
    case 'initialize':
      return reply({
        ...
        capabilities: { tools: {} },
        serverInfo: { name: 'bookrank', version: VERSION },
      });
    ...
    case 'tools/call': {
      ...
      try {
        ...
        if (err instanceof ToolError) {
          return reply({ content: [{ type: 'text', text: `bookrank: ${err.message}` }], isError: true });
        }
        throw err;
      }
      if (!result) {
