...
- [x] Living Well in a Down Economy: tips #41–175 (all remaining chapters) fully summarized 2026-07-26 — book complete, needs `sync-summaries.sh` + badge
...
- [x] **Screenshots** — set up `fastlane snapshot` from scratch (Snapfile, UITests/PreviewScreenshot.swift, shared Xcode scheme in project.yml), captured on iPhone 11 Pro Max + 14 Plus, verified real content rendered (not blank), uploaded via `asc screenshots upload` (both landed in the APP_IPHONE_65 bucket — 14 Plus sim's resolution matches 6.5", not 6.7"). Blocking errors now down to 1.
