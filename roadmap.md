...
- [x] "Asc issues" (note "Bookrank") — FIXED 2026-08-09. Root cause confirmed: iOS 1.0 submission `01f1f74b` was REJECTED with the phantom orphaned `inAppPurchaseVersion` pattern (app has 0 real IAPs, same as sparkjar). By the time of the fix pass, `asc validate iap`/`asc validate --version-id` both came back clean (0 errors, 0 blocking) — the phantom item had already fallen off, or app-level IAP state had cleared since the rejection. Build currency checked: build 6 (uploaded 2026-08-03) is still the current build; iOS localization keywords already reflect the Bookrank rename (2026-08-07), so no stale-branding risk. Fix applied: cancelled the old `UNRESOLVED_ISSUES` submission `01f1f74b`, resolved a leftover empty draft submission `689d674d` (add version item to it since it couldn't be cancelled once non-empty), `asc review submissions-submit --id 689d674d --confirm`. iOS 1.0 is now **WAITING_FOR_REVIEW** (submission `689d674d`, confirmed via `asc review submissions-list`). macOS 1.0 unaffected, still `WAITING_FOR_REVIEW` (`bc9a7d5f`).
